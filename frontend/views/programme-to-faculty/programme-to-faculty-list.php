...
<?php 
$this->title = 'Programme-Faculty-List';
echo "<h1 class='box-title'>$this->title </h1>"; ?>
<?php if (Yii::$app->session->getFlash('programmerofacultyexists') || Yii::$app->session->getFlash('programmerofacultysaved') || Yii::$app->session->getFlash('programmerofacultyupdated') || Yii::$app->session->getFlash('programmerofacultydeleted')) { 
if(Yii::$app->session->getFlash('programmerofacultyexists')){
	$issuccess = 'Failed!';
	$msg = Yii::$app->session->getFlash('programmerofacultyexists');
}else if(Yii::$app->session->getFlash('programmerofacultysaved')){
	$issuccess = 'Success!';
	$msg = Yii::$app->session->getFlash('programmerofacultysaved');
}else if(Yii::$app->session->getFlash('programmerofacultyupdated')){
	$issuccess = 'Success!';
	$msg = Yii::$app->session->getFlash('programmerofacultyupdated');
}else if(Yii::$app->session->getFlash('programmerofacultydeleted')){
	$issuccess = 'Success!';
	$msg = Yii::$app->session->getFlash('programmerofacultydeleted');
}
?>
...
<?php $form = ActiveForm::begin(); ?>
        <div class="col-sm-2 col-xs-12 p-left0 ad-lst">
            <?php echo $form->field($model, 'faculty_name')->textInput(array('placeholder' => 'Faculty Name'), ['class' => 'form-control']) ?>
        </div>
        <div class="col-sm-2 col-xs-12 p-left0 ad-lst">
        ...
        <div class="searchBtn">
                ...
                <input type="hidden" value="<?php echo Yii::$app->request->BaseUrl; ?>/../../programme-to-faculty-list" id="searchUrl">
            </div>
		 ...
		 [
            ...
            ['class' => 'yii\grid\ActionColumn',
                            ...
                            'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'update') {
                                    return Url::toRoute(['../../update-programme-to-faculty', 'id' => $model['id']]);
                                }else if($action === 'delete'){
                                    return Url::toRoute(['delete-programme-to-faculty', 'id' => $model['id']]);
                                }
                            } 
                        ],
    ],
...
var faculty_name = "<?php echo !empty(Yii::$app->getRequest()->getQueryParam('faculty_name')) ? Yii::$app->getRequest()->getQueryParam('faculty_name') : '' ?>";
	$('#assignprogrammefaculty-programme_name').val(programme_name);
    $('#assignprogrammefaculty-faculty_name').val(faculty_name);
$('#btnSearch').on('click', function (e) {
            var searchUrl = $('#searchUrl').val();
            var pjaxContainer = 'pjax-list';
            var programme_name = $('#assignprogrammefaculty-programme_name').val();
            var faculty_name = $('#assignprogrammefaculty-faculty_name').val();
                var pjaxReloadURL = searchUrl + '?programme_name=' + programme_name+ '&faculty_name=' + faculty_name;

            $.ajax({
                url: searchUrl,
                type: 'get',
                data: {'programme_name': programme_name, 'faculty_name': faculty_name},
...
$('#btnReset').on('click', function (e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data) {
                        $('#assignprogrammefaculty-faculty_name').val('');
                        $('#assignprogrammefaculty-programme_name').val('');
                        //$.pjax.reload({url: pjaxReloadURL, container: '#' + $.trim(pjaxContainer, )});
                        $.pjax.reload({url: pjaxReloadURL, container: '#' + $.trim(pjaxContainer), async: false});
                        return false;
                    }
