...
<?php 
$this->title = 'Faculty List';
echo "<h1 class='box-title'>$this->title </h1>"; ?>
<?php if (Yii::$app->session->getFlash('facultyexists') || Yii::$app->session->getFlash('facultysaved') || Yii::$app->session->getFlash('facultyupdated') || Yii::$app->session->getFlash('facultydeleted')) { 
if(Yii::$app->session->getFlash('facultyexists')){
	$issuccess = 'Failed!';
	$msg = Yii::$app->session->getFlash('facultyexists');
}else if(Yii::$app->session->getFlash('facultysaved')){
	$issuccess = 'Success!';
	$msg = Yii::$app->session->getFlash('facultysaved');
}else if(Yii::$app->session->getFlash('facultyupdated')){
	$issuccess = 'Success!';
	$msg = Yii::$app->session->getFlash('facultyupdated');
}else if(Yii::$app->session->getFlash('facultydeleted')){
	$issuccess = 'Success!';
	$msg = Yii::$app->session->getFlash('facultydeleted');
}
?>
...
<?php $form = ActiveForm::begin(); ?>
        ...
        <div class="searchBtn">
                ...
                <input type="hidden" value="<?php echo Yii::$app->request->BaseUrl; ?>/../../faculty-list" id="searchUrl">
            </div>
		 ...
		 [
            ...
            ['class' => 'yii\grid\ActionColumn',
                                        ...
                                        'update' => function ($url, $model) {
                                         ...
                                         'delete'=>function ($url, $model) { 
                                            ...
                                            return HTML::a('<span class="glyphicon glyphicon-trash"></span>',$url,[
                                                ...
                                                'data-confirm'=>'Are you sure you want to delete this Faculty?',
                                            ]);                               
                                                ...
                                                return HTML::a('<span class="glyphicon glyphicon-repeat"></span>',$url,[
                                                    ...
                                                    'delete-url' => $url, 
                                                    'pjax-container' => 'pjax-list',
                                                    'data-confirm'=>'Are you sure you want to undo delete this Faculty?',
                                                ]);        
                                            }
                                          },
                                      ],
                            'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'update') {
                                    return Url::toRoute(['../../update-faculty', 'id' => $model['id']]);
                                }else if($action === 'delete'){
                                    return Url::toRoute(['faculty-delete', 'id' => $model['id'], 'status' => $model['status']]);
                                }
                            } 
                        ],
    ],
...
var faculty_name = "<?php echo !empty(Yii::$app->getRequest()->getQueryParam('faculty_name')) ? Yii::$app->getRequest()->getQueryParam('faculty_name') : '' ?>";
	$('#faculty-faculty_name').val(faculty_name);
$('#btnSearch').on('click', function (e) {
            var searchUrl = $('#searchUrl').val();
            var pjaxContainer = 'pjax-list';
            var faculty_name = $('#faculty-faculty_name').val();
                var pjaxReloadURL = searchUrl + '?faculty_name=' + faculty_name;
